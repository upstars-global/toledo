...
if (typeof require !== "undefined") {
    LCS_DIFF_ARRAY_METHOD = require("assets/js/diff").diffArrays;
} else {
    try {
    ...
    for (let columnIndex = 0; columnIndex < columnDiffs.length; columnIndex++) {
        ...
        for (let depthIndex = 0; depthIndex < columnDiff.length; depthIndex++) {
            ...
            if (columnDiff[depthIndex].removed) {
                ...
                removedCounter = removedCounter + segmentLength;
                ...
                if (removedCounter > 0) {
                    if (segmentLength > removedCounter) {
                        segmentLength = segmentLength - removedCounter;
                        removedCounter = 0;
                    } else {
                        removedCounter = removedCounter - segmentLength;
                        segmentLength = 0;
...
function groupAdjacent(columns, spread, h, w) {
     ...
     * @returns {[array]}         [0] lower bound, [1] upper bound
     */
    ...
    while (columnPointer < w) {
        ...
        columnPointer = columnPointer + spread;
    }
    ...
    if (rowSpread > 1) {
        for (let index = 0; index < groupedColumns.length; index++) {
            ...
            while (depthPointer < h) {
                ...
                depthPointer = depthPointer + rowSpread;
            }
