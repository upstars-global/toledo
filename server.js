...
app.get("/api/test-list", (req, res) => {
    const tests = [];
    fs.readdir(path.join(__dirname, `bitmaps_test/${ req.query.project }`), null, (err, files) => {
        if (err) {
            throw err;
        }
...
app.get("/config.js", (req, res) => {
    ...
    fs.readFile(path.join(
        __dirname,
        `bitmaps_test/${ referrer.searchParams.get("project") }`,
...
app.use("/bitmaps_test", express.static("bitmaps_test"));
app.use("/bitmaps_reference", express.static("bitmaps_reference"));
